...
int supakilla::count=-1;
int supakilla::spiral_safe=1;
int supakilla::spiral_wont_work()
{
    spiral_safe=0;
}
int supakilla::getcount()
{
    return ++count;
}

int supakilla::SNM(char board[BOARD_LENGHT][BOARD_BREADTH],int head_positiony,int head_positionx ,int acc_direction,int life_left)
{
...
bool supakilla::checkvoid(char board[BOARD_LENGHT][BOARD_BREADTH],int heady,int headx,int dir)
{
	int up,down;
	if(dir==LEFT)
	{
...
public:   
	...
	int getnewmove(char board[BOARD_LENGHT][BOARD_BREADTH],int head_positiony,int head_positionx ,int acc_direction,int life_left){
		...
		if (phase==5){
			...
			if (VRBS) std::cout<<"OUR POSX="<<head_positionx<<" POSY="<<head_positiony<<"\n";
